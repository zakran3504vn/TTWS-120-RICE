...
if (isset($_POST['create_bank'])) {
    ...
    if (empty($errors)) {
        $sql = "INSERT INTO banking (bankName, bankNumber, accountHolder, status, create_at) VALUES (?, ?, ?, ?, NOW())";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("sssi", $bank_name, $bank_number, $account_holder, $status);

        if ($stmt->execute()) {
            $success_message = "Tài khoản ngân hàng đã được tạo thành công!";
        } else {
...
<head>
    ...
    <title>Thêm Tài Khoản Ngân Hàng</title>
    <!-- Bootstrap CSS -->
                            ...
                            <div class="QA_table mb_30">
                                <div class="table-responsive">
                                    <div class="container mt-5">
                                        <h2 class="text-center mb-4">Thêm tài khoản ngân hàng</h2>
                                        <div class="form-container">
                                            <form action="" method="POST" class="p-4 border rounded shadow">
                                                ...
                                                <div class="mb-3">
                                                    ...
                                                    <input type="text" id="bank_number" name="bank_number" class="form-control" placeholder="Nhập số tài khoản" required>
                                                </div>

                                                <div class="mb-3">
                                                    <label for="account_holder" class="form-label">Chủ Tài Khoản:</label>
                                                    <input type="text" id="account_holder" name="account_holder" class="form-control" placeholder="Nhập tên chủ tài khoản" required>
                                                </div>
                                                ...
                                                <button type="submit" class="btn btn-primary" name="create_bank">Tạo tài khoản</button>
                        ...
                        if (data.code === "00" && data.data) {
                            return {
                                results: data.data.map(function(bank) {
                                    return {
                                        id: bank.code, // Use bank code instead of name
                                        text: bank.shortName,
                                        logo: bank.logo,
                                        name: bank.name
                                    };
